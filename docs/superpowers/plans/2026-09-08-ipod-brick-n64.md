...
Also wrap the whole `read_input` function definition in `#ifndef BRICK_AUTOPLAY` / `#endif` so the autoplay build does not warn about an unused function.
...
Run: `scripts/ares-shot.sh brick-autoplay.z64 build/shots/task7 3 20 60 332 345 390`
Expected (autoplay clears level 1 after 19,569 ticks, about 326 s at 60 Hz, then holds the paddle still): 3 s shows the title or the first serve; 20 s and 60 s show play with a shrinking grid; 332 s shows level 2 begun (full grid, "LV 2"); 345 s shows lives dropping; 390 s shows "GAME OVER" with "HIGH SCORE" at least 60. If 390 s is still level 2 in progress, extend with `450 510`. The run keeps ares open for about six and a half minutes.
...
Added after Task 7 found that the emulator run diverged from the host simulation and stalled on two corner bricks.

**Files:**
- Modify: `src/brick.c` (the PLAY branch of `brick_autoplay_input` only)
- Modify: `tests/test_brick.c` (tighten one bound)
- Modify: `src/n64/app.c` (main loop only)

**Interfaces:**
- Consumes: `brick_cell_rect`, `brick_autoplay_input`.
- Produces: an autoplay run whose tick sequence is identical on the host and in ares, clearing level 1 in about 10,300 ticks.

- [ ] **Step 1: Tighten the autoplay test bound**

In `test_autoplay_clears_level_and_ends`, change `t < 30000` to `t < 15000` in the first loop (level 2 must arrive within 15,000 ticks). Leave the 60,000 game-over bound.

- [ ] **Step 2: Run the tests to verify the tightened test fails**

Run: `make test`
Expected: `test_autoplay_clears_level_and_ends` prints `FAILED` (the current policy needs 19,569 ticks); exit 1.

- [ ] **Step 3: Replace the PLAY branch of `brick_autoplay_input`**
...
Run: `make test`
Expected: 24 ok; the autoplay test prints `level 2 after 10269 ticks (171 s)`.

- [ ] **Step 5: Compute autoplay input per tick in the adapter**

In `src/n64/app.c` `main`, change the loop so that the autoplay build recomputes its input inside the catch-up loop while the normal build keeps sampling the controller once per frame:
...
- [ ] **Step 6: Rebuild and recapture on the new schedule**

Run: `make rom && make rom-autoplay`
Run: `scripts/ares-shot.sh brick-autoplay.z64 build/shots/task7b 3 60 173 175 200`
Expected (level 2 begins at tick 10,269 ≈ 171 s after the game starts, about 1 s after ares boots; game over at tick 10,579 ≈ 176 s): 3 s and 60 s show play; 173 s and 175 s show level 2 (full or nearly full grid, "LV 2", score at least 60) with lives dropping; 200 s shows "GAME OVER", "HIGH SCORE 63", "PRESS A". This run takes under four minutes.
...
Added on request: the builtin debug font is hard to read. Uses Inter Bold (SIL Open Font License 1.1), already committed at `assets/Inter-Bold.ttf` with `assets/LICENSE-Inter.txt`.

**Files:**
- Modify: `Makefile` (font conversion and DragonFS inside the `ifdef N64_INST` block; `clean`)
- Modify: `.gitignore` (add `filesystem/`)
- Modify: `src/n64/app.c` (filesystem init, two fonts, text baselines, video filter)
- Modify: `README.md` (Fonts line)

**Interfaces:**
- Consumes: `assets/Inter-Bold.ttf`; libdragon `mkfont`, `mkdfs`, `dfs_init`, `rdpq_font_load`.
- Produces: `filesystem/hud.font64` (12 px) and `filesystem/big.font64` (22 px) embedded in both ROMs; font ids `FONT_HUD` (1) and `FONT_BIG` (2).

- [ ] **Step 1: Makefile**

Inside the `ifdef N64_INST` block, after the `OBJS :=` line, add:
...
Change `clean` to `$(RM) -r build filesystem *.z64`. Add `filesystem/` to `.gitignore`. Recipe lines need real tabs. `mkfont` does not create output directories, hence the `mkdir -p`.

- [ ] **Step 2: Adapter changes in `src/n64/app.c`**

Replace `#define FONT_ID 1` with:
...
In `main`, call `dfs_init(DFS_DEFAULT_LOCATION);` as the first statement, change the display filter to `FILTERS_DISABLED`, and replace the builtin font block with:
...
In `render`, use `FONT_HUD` for the three HUD strings at baseline y 26 (the 12 px font's cap height keeps the glyph tops below y 16), and `FONT_BIG` for everything else with these baselines: title "BRICK" 100, "HIGH SCORE %d" 130, "PRESS A" 160; "PAUSED" 160; game over "GAME OVER" 140, "HIGH SCORE %d" 168, "PRESS A" 196. All big-font lines sit between the grid bottom (y 107) and the paddle (y 218), and 22 px lines need at least 28 px spacing.

- [ ] **Step 3: README**

Add under the first paragraph: `Text is set in [Inter](https://rsms.me/inter/) (SIL Open Font License 1.1, see assets/LICENSE-Inter.txt).`

- [ ] **Step 4: Build and verify**

Run: `make rom && make rom-autoplay`
Expected: `[FONT]`-style mkfont output appears once per font, `[DFS]` lines appear, both ROMs build with no warnings, and each ROM is roughly 12 KB larger than before.

Run: `scripts/ares-shot.sh brick.z64 build/shots/task8-title 4` and `scripts/ares-shot.sh brick-autoplay.z64 build/shots/task8-play 8`
Expected: crisp, unblurred pixels; title text in a clean large sans-serif; HUD text small but clearly legible; nothing overlapping the grid or paddle; HUD glyphs not closer than about 14 px to the top edge.
...
- [ ] **Step 1b: Hide the ball on the game-over screen**

In `render`, draw the ball only when `g->state != BRICK_ST_GAMEOVER`, so the lost ball is not left sitting below the paddle under the overlay.
...
Sections: What it is (one paragraph, credit Steve Wozniak's original and libdragon), Fonts (Inter, SIL Open Font License 1.1, `assets/LICENSE-Inter.txt`), Requirements (Docker Desktop, ares via `brew install --cask ares-emulator`, clang), Build, Run, Controls, Develop (`make test`, `make frames`, `make rom-autoplay`, `make shots`, `make rom DEBUG=1`), Layout constants (point to `src/brick.h`), and a Verification note explaining that mupen64plus cannot boot libdragon ROMs. Record the Docker base image digest: run `docker image inspect ghcr.io/dragonminded/libdragon:latest-arm64 --format '{{index .RepoDigests 0}}'` and paste it.
...
git commit -m "Task 9: debug build flag, overscan check, and complete README"
```
